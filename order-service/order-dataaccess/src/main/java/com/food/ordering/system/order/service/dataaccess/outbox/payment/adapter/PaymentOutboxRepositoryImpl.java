...
public class PaymentOutboxRepositoryImpl implements PaymentOutboxRepository {
    ...
    private final PaymentOutboxDataAccessMapper dataMapper;

    public PaymentOutboxRepositoryImpl(PaymentOutboxJpaRepository jpaRepository,
                                       PaymentOutboxDataAccessMapper dataMapper) {
    ...
    public OrderPaymentOutboxMessage save(OrderPaymentOutboxMessage orderPaymentOutboxMessage) {
        return dataMapper.paymentOutboxEntityToOrderPaymentOutboxMessage(
                jpaRepository.save(dataMapper.orderPaymentOutboxMessageToOutboxEntity(orderPaymentOutboxMessage)));
    }
    ...
    public Optional<List<OrderPaymentOutboxMessage>> findByTypeAndOutboxStatusAndSagaStatus(String sagaType,
        ...
        return Optional.of(
                    jpaRepository.findByTypeAndOutboxStatusAndSagaStatusIn(sagaType,
                            ...
                            .orElseThrow(
                                    () -> new PaymentOutboxNotFoundException("Payment outbox object " +
                                            "could not be found for saga type " + sagaType))
                            .stream()
                            .map(dataMapper::paymentOutboxEntityToOrderPaymentOutboxMessage)
